...
function setupOverlayCanvas(videoElement) {
    ...
    const updateSize = () => {
        if (videoElement.videoWidth > 0) {
            ...
            console.log("Canvas size set:", videoElement.videoWidth, "x", videoElement.videoHeight);
        } else {
...
function startDetectionLoop(videoElement) {
    ...
    function detect() {
        if (!isDetecting || !videoElement || videoElement.paused || videoElement.ended) {
            return;
        }
        ...
        if (videoElement.currentTime !== lastVideoTime) {
            ...
            try {
                const result = faceDetector.detectForVideo(videoElement, performance.now());
...
function drawBoundingBoxes() {
    ...
    detectionResults.forEach(detection => {
        ...
        // Flip horizontally to match CSS scaleX(-1) on video
